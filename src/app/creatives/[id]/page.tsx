...
export default async function CreativePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: creative } = await supabase
    .from("creatives")
    .select(
      "id, org_id, sheet_id, ad_name, content_summary, hook_line, hypothesis, hook_angle, archetype, feature_pillar, sport, format, variant_differentiator, cta, status, idea_status, is_proven, compliance_note, script_doc_url, cpt_target_cents, concept_families(name, compliance_note)",
    )
    .eq("id", id)
    .single();
        ...
        <aside className="flex flex-col gap-4 lg:sticky lg:top-20">
          ...
          {staff && creative.ad_name && (
            <div className="rounded-[14px] border border-white/[0.09] bg-white/[0.025] p-4">
              ...
              <PerformanceQuickEntry orgId={creative.org_id} adName={creative.ad_name} />
            </div>
          )}
