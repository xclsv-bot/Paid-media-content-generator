...
import { VERDICTS, type Verdict } from "@/lib/metrics/verdict";

// The report's verdict vocabulary IS the app's (src/lib/metrics/verdict.ts) —
// one home, so the sheet parser and the loop can't drift apart.
...
export const REPORT_VERDICTS: ReadonlySet<string> = new Set(VERDICTS);
