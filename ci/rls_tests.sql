...
-- review-feedback fixtures (0033): a staff comment + approval on the concept
-- assigned to creator1 (aaaa…) and on the UNASSIGNED client-org concept
-- (abab…, 'Outlier concept 2' above). The unassigned probe must be client-org:
-- an agency-org creative (like bbbb…) is already creator-visible via
-- can_see_creative's org match — pre-existing behavior, not the grant under test.
...
-- 19) review feedback: the ASSIGNED creator reads the comment + approval on
-- their concept; an unassigned creator sees neither; creator writes stay
-- blocked (SELECT-only grant).
do $$
declare n int;
begin
  perform set_config('request.jwt.claims', '{"sub":"33333333-3333-3333-3333-333333333333"}', true);
  ...
  raise notice 'ok - assigned creator reads review feedback; unassigned creator sees none (0033)';
end $$;
do $$
begin
  perform set_config('request.jwt.claims', '{"sub":"33333333-3333-3333-3333-333333333333"}', true);
