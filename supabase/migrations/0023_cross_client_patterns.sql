-- ============================================================================
-- 0023_cross_client_patterns.sql — the actual cross-client sharing mechanism
...
create table public.cross_client_patterns (
  ...
  title                text not null,
  pattern_type         text not null default 'hook',    -- constrained below
  ...
  status               text not null default 'draft',     -- constrained below; mirrors the newer stores' posture
  ...
  constraint ccp_type_check   check (pattern_type in ('hook', 'family_archetype', 'cta', 'structure')),
  constraint ccp_status_check check (status in ('draft', 'published', 'archived'))
);
